...
local weaponStatsMode = nil  -- "Rage" veya "Legit"
...
local chamsCharacterConnections = {}

-- Wallbang için orijinal fonksiyon saklama
...
local function buySeeds(seedType, count)
	...
	task.wait(1)
	local remote = ReplicatedStorage:FindFirstChild("SeedDealer")
	if remote then
		for i = 1, count do
	...
	if char and char:FindFirstChild("HumanoidRootPart") then
		local root = char.HumanoidRootPart
		local origPos = originalCFrame.Position
		local groundY = getGroundHeight(origPos.X, origPos.Z)
		local newY
		if groundY then
			newY = groundY + 3
		else
		...
		local newPos = Vector3.new(origPos.X, newY, origPos.Z)
		root.CFrame = CFrame.new(newPos) * (originalCFrame - originalCFrame.Position)
		root.Velocity = Vector3.zero
		root.RotVelocity = Vector3.zero
	end
end
...
-- ============================================
-- RAGE / LEGIT WEAPON HACKS
-- ============================================

-- Rage modunda bir silaha uygulanacak tüm ayarlar
local function applyRageStats(tool)
	...
	if ms then
		...
		elseif type(ms) == "number" then stats.MaxSpread = 0
		end
	end
	...
	if humanoid then
		...
		if animator then
			for _, track in pairs(animator:GetPlayingAnimationTracks()) do
				if string.find(track.Name, "Recoil") then
					track:Stop()
				end
			end
		end
...
local function disableWallbang()
	if originalFindPartOnRay then
		Workspace.FindPartOnRayWithIgnoreList = originalFindPartOnRay
...
local function createGUI()
	...
	-- ========== RAGE / LEGIT WEAPON (radio) ==========
	local weaponFrame = Instance.new("Frame", scrollFrame)
	weaponFrame.Size = UDim2.new(1,-10,0,35)
	weaponFrame.BackgroundTransparency = 1
	weaponFrame.LayoutOrder = 13
	...
	local rageBtn = createRadioButton(weaponFrame, "Rage", false, 0.35)
	local legitBtn = createRadioButton(weaponFrame, "Legit", false, 0.65)
	...
	rageBtn.MouseButton1Click:Connect(function() setWeaponMode("Rage") end)
	table.insert(allConnections, rageBtn.MouseButton1Click)

	legitBtn.MouseButton1Click:Connect(function() setWeaponMode("Legit") end)
	table.insert(allConnections, legitBtn.MouseButton1Click)
